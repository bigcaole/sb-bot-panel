...
ADMIN_SECURITY_EVENTS_CACHE_TTL_SECONDS="5"
...
load_existing_env_defaults() {
  ...
  old_admin_security_events_cache_ttl="$(get_env_value "ADMIN_SECURITY_EVENTS_CACHE_TTL_SECONDS")"
  ...
  ADMIN_SECURITY_EVENTS_CACHE_TTL_SECONDS="${old_admin_security_events_cache_ttl:-5}"
...
normalize_loaded_values() {
  ...
  if ! [[ "$ADMIN_SECURITY_EVENTS_CACHE_TTL_SECONDS" =~ ^[0-9]+$ ]]; then
    ADMIN_SECURITY_EVENTS_CACHE_TTL_SECONDS="5"
  fi
  if (( ADMIN_SECURITY_EVENTS_CACHE_TTL_SECONDS > 300 )); then
...
# 管理端安全事件统计缓存秒数（0=关闭缓存）
ADMIN_SECURITY_EVENTS_CACHE_TTL_SECONDS=${ADMIN_SECURITY_EVENTS_CACHE_TTL_SECONDS}
...
run_self_checks() {
  ...
  check_env_key "ADMIN_SECURITY_EVENTS_CACHE_TTL_SECONDS"
...
show_summary() {
  ...
  echo "ADMIN_SECURITY_EVENTS_CACHE_TTL_SECONDS: ${ADMIN_SECURITY_EVENTS_CACHE_TTL_SECONDS}"
