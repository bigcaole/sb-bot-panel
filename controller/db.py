...
def init_db() -> None:
    ...
    with get_connection() as conn:
        ...
        conn.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_audit_logs_action_source_ip_created_at
            ON audit_logs(action, source_ip, created_at DESC)
            """
        )
        conn.execute(
            """
        ...
        conn.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_audit_logs_action_actor_created_at
            ON audit_logs(action, actor, created_at DESC)
            """
        )
        conn.execute(
            """
