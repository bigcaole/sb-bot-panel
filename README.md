...
15. SSH 安全状态总览（只读）
16. 启用 SSH 仅密钥登录（禁用密码）
17. 恢复 SSH 密码登录（应急）
【系统级操作（谨慎）】
18. 更新同步（保留原配置，自动 `git pull`，无交互）
19. 卸载
20. 退出

说明：

- 首次安装后，后续更新建议直接用菜单 `18`，不会重复询问端口/域名等参数。
- 需要改参数时用菜单 `1`。
- 安全建议：先用菜单 `14` 生成并部署公钥，再用菜单 `15` 做只读安全检查，最后执行菜单 `16` 启用仅密钥登录。
- 菜单 `15` 会输出 SSH 风险等级（低/中/高）和修复建议清单；启用仅密钥登录前脚本也会做防锁死检测（当前会话来源 IP、fail2ban 封禁、UFW SSH 放行）。
- 也可命令行直接执行：
