...
bot「用户管理」已支持“编辑信息”和“用户续期”，普通用户与过期用户都可直接操作。
...
若 TUIC/ACME 导致 sing-box 重载失败，agent 会自动暂停 TUIC 并降级为仅 VLESS，避免整机代理中断。
菜单更新完成后会明确回显：`升级结果` 与 `当前项目版本（VERSION + commit）`。
...
- 菜单默认是“简化视图”（仅核心流程）。
- 日志/迁移/卸载/高级安全工具已下沉到高级视图，避免新手被“功能墙”淹没。
- 输入 `A` 可切到“高级视图”（显示全部功能），在高级视图输入 `B` 可切回简化视图。

- 首次安装后，后续更新建议直接用菜单 `21`，不会重复询问端口/域名等参数。
- 菜单 `21` 若拉取失败会直接中止并提示，不会继续执行后续更新步骤。
- 菜单 `21` 更新成功后会输出升级结果和当前版本号（`VERSION + commit`），便于快速验收。
...
- 管理菜单默认是“简化视图”（仅核心流程）。
- 日志/迁移/卸载/高级安全工具已下沉到高级视图。
- 输入 `A` 可切换到“高级视图”（全功能），在高级视图输入 `B` 可切回简化视图。
